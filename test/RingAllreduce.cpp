...
int main(){
    ...
    const int size = 1000; // size should be divisible by numRanks for test to pass!
    ...
    // setup Allreduce
    std::cout<<"Building RingAllreduce"<<std::endl;
    Comprex_RingAllreduce<data_t> allreduce(&gaspi_env);

    // define connectivity pattern
    std::cout<<"Connecting Nodes"<<std::endl;
    float size_factor = 3.0;
    allreduce.setupConnections(size, tag, compression_ratio, size_factor);
    ...
    for(int run = 0; run<num_runs; run++) { 
        ...
        for(int it=0; it<size; ++it){
            if(it>=val_start && it<val_stop) continue;
            result[it] = values[it];
        }
